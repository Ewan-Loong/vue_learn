<template>
  <div class="user-list-container">
    ...
    >
      <a-descriptions
          v-if="currentEditRecord && formItemsForDrawer.length"
          bordered
          size="middle"
          :column="1"
      >
        <a-descriptions-item
            v-for="col in formItemsForDrawer"
            :key="col.dataIndex || col.key"
            :label="col.title"
        >
          {{ formatValue(currentEditRecord[col.dataIndex], col.type) }}
        </a-descriptions-item>
      </a-descriptions>
      <div v-else>
        <p>暂无详情信息</p>
...
export default {
  name: "UserList",
  components: {GenericTable, DrawerForm, FileUpload},
  data() {
    return {
      loading: false,
      tableData: [],
  ...
  methods: {
    async refreshData() {
      this.loading = true;
      try {
        await this.$api.post('UserQuery').then(response => {
